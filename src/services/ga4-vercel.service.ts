...
class Ga4VercelService {
  ...
  constructor() {
    ...
    try {
      // For Vercel, we'll use the JSON credentials directly
      const credentialsJson = this.getCredentialsFromEnv();
      if (credentialsJson) {
        console.log('Using JSON credentials for GA4');
...
export const ga4VercelService = new Ga4VercelService();
