...
class Ga4VercelIntegrationService {
  ...
  constructor() {
    ...
    try {
      // For Vercel, we need to use explicit credentials
      const credentials = this.getVercelCredentials();
      console.log('GA4VercelIntegrationService: Initializing with Vercel credentials');
      this.client = new BetaAnalyticsDataClient(credentials);
    } catch (error) {
      console.error('GA4VercelIntegrationService: Failed to initialize GA4 client:', error);
    }
  }

  private getVercelCredentials() {
    // Check if we have the JSON credentials in environment
    if (process.env.GOOGLE_APPLICATION_CREDENTIALS_JSON) {
      try {
        console.log('GA4VercelIntegrationService: Using GOOGLE_APPLICATION_CREDENTIALS_JSON');
        const credentialsJson = JSON.parse(process.env.GOOGLE_APPLICATION_CREDENTIALS_JSON);
        return { credentials: credentialsJson };
      } catch (error) {
        console.error('GA4VercelIntegrationService: Failed to parse credentials JSON:', error);
      }
    }

    // Fallback: try to use default credentials (for local development)
    console.log('GA4VercelIntegrationService: Using default credentials');
    return {};
  }
  ...
  async getMetrics(dateRange: { startDate: string; endDate: string }): Promise<Ga4MetricsResponse> {
    ...
    try {
      console.log(`GA4VercelIntegrationService: Fetching data for property: ${this.propertyId}`);
      console.log(`GA4VercelIntegrationService: Date range: ${dateRange.startDate} to ${dateRange.endDate}`);
      ...
      console.error('GA4VercelIntegrationService: Error fetching GA4 metrics:', error);
      return {
        data: { sessions: 0, totalUsers: 0, pageviews: 0, averageEngagementTime: 0 },
...
export const ga4VercelIntegrationService = new Ga4VercelIntegrationService();
