...
class Ga4OAuth2Service {
  ...
  private async getAccessToken(): Promise<string | null> {
    try {
      ...
      if (token) {
        console.log('GA4OAuth2Service: Using GOOGLE_ACCESS_TOKEN from environment');
        return token;
      }

      // If no token available, return null
      console.log('GA4OAuth2Service: No access token available');
      return null;
    } catch (error) {
      console.error('GA4OAuth2Service: Failed to get access token:', error);
      return null;
    }
  }
  ...
  async getMetrics(dateRange: { startDate: string; endDate: string }): Promise<Ga4MetricsResponse> {
    ...
    if (!accessToken) {
      return {
        data: { sessions: 0, totalUsers: 0, pageviews: 0, averageEngagementTime: 0 },
        error: 'No Google access token available. Please set GOOGLE_ACCESS_TOKEN environment variable.'
      };
    }

    try {
      console.log(`GA4OAuth2Service: Fetching data for property: ${this.propertyId}`);
      console.log(`GA4OAuth2Service: Date range: ${dateRange.startDate} to ${dateRange.endDate}`);
      ...
      if (!response.ok) {
        ...
        console.error('GA4OAuth2Service: API request failed:', response.status, errorText);
        return {
          data: { sessions: 0, totalUsers: 0, pageviews: 0, averageEngagementTime: 0 },
      ...
      console.error('GA4OAuth2Service: Error fetching GA4 metrics:', error);
      return {
        data: { sessions: 0, totalUsers: 0, pageviews: 0, averageEngagementTime: 0 },
...
export const ga4OAuth2Service = new Ga4OAuth2Service();
